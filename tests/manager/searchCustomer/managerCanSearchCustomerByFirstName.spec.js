...
test('Assert manager can search customer by First Name', async ({ page }) => {
  ...
  */
  ...
  await addCustomerPage.openCustomers();
  await addCustomerPage.fillSearchCustomer(customer.firstName);
  await addCustomerPage.assertCustomerRowIsVisibe(customer.firstName);
  await addCustomerPage.assertCustomerTableRowCount(1);
});
